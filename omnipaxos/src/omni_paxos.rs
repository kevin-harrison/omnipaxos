use crate::{
    ballot_leader_election::{Ballot, BallotLeaderElection, NodeLatencies},
    ...
    B: Storage<T>,
{
    ...
    pub fn is_accept_phase_leader(&self) -> bool {
        self.seq_paxos.is_accept_phase_leader()
    }
    ...
    pub fn relinquish_leadership(&mut self, to: NodeId) {
        ...
        if let Some(_can_transfer_leadership) = self.seq_paxos.relinquish_leadership(to) {
            // NOTE: switched to no leader election so this is unnecessary for now
    ...
    pub fn election_timeout(&mut self) -> Vec<Option<NodeLatencies>> {
