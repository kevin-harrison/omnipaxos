...
fn sync_test(test: SyncTest) {
    ...
    if followers_new_decided_len == 0 {
        // Wait a preset time, hopefully AccSync finishes by then
        sys.set_node_connections(follower_id, true);
        let wait_timeout = Duration::from_millis(800);
        std::thread::sleep(wait_timeout);
    } else {
        // Wait for new entries from AccSync to be decided so we can verify log
